#!/usr/bin/env bash
# Full competitive comparison over the same TPC-H data: lakehouse-engine, native IMPORT ceiling,
# Athena, optionally Trino (native + IMPORT FROM JDBC) and Spark. Aggregates every report's
# "TIMING <engine> <query> <seconds>" lines into one summary table.
#
# Trino native and IMPORT FROM JDBC each get their own freshly booted cluster so neither
# measurement JIT-warms or caches manifests for the other. That doubles the AWS spend, hence the
# RUN_TRINO_COMPARISON=1 opt-in; it also needs the Trino EC2 key pair's private key locally.
# Spark is never provisioned here; it is skipped when EMR_SERVERLESS_APP_ID is unset.
set -euo pipefail
cd "$(dirname "$0")/.."
