...
// Defaults to twice the detected per-node core count, floored at
...
/// Minimum parallelism factor, applied as a floor so a small detected core count
/// never collapses the oversubscription multiplier below a useful value.
...
/// DF_TARGET_PARTITIONS, DF_THREADS_PER_UDF, DF_THREADING_MODE, DF_BATCH_SIZE,
...
/// the request are preserved (merge, not clobber) — including a key the adapter
/// no longer writes, such as an NR_OF_CORES entry inherited from a virtual
/// schema created before the core count became a per-request detection.
...
/// floored at `DEFAULT_PARALLELISM_FACTOR` so a small node, such as a dev VM,
/// still oversubscribes enough to keep the shard fan-out useful.
...
/// `DATAFUSION_TARGET_PARTITIONS` / `DATAFUSION_THREADS_PER_UDF` values. On a
/// single-core node both fields come out as `1`.
...
///
/// `max(1, floor(nr_of_cores / udf_instances_per_node))`. The floor guarantees
/// the non-oversubscription invariant `udf_instances_per_node × threads ≤
/// nr_of_cores` whenever the node has at least as many cores as instances; when
/// instances exceed cores the `max(1, …)` floor
...
/// the detected core count, and a single-core node lands on a single partition
/// or thread.
...
///   the per-node UDF-instance share. The derivation is uniform at every node
///   size — the smallest node simply lands on one thread's worth of connections
///   — so there is no separate default-budget branch.
///
/// # AUTO formula
///
...
///
/// Detected from `std::thread::available_parallelism()`, which on Linux honours
/// the cgroup CPU quota the adapter's VM runs under, so the count reflects the
/// capacity actually granted to the node rather than the bare hardware. It is
/// resolved once per createVirtualSchema request and threaded into every
/// derivation, which keeps each derivation a pure function of an injected count
/// rather than a reader of ambient state.
fn resolve_nr_of_cores() -> u32 {
    core_count_or_default(std::thread::available_parallelism())
}

/// Map a core-count detection result onto a usable count, substituting a single
/// core when the platform cannot report one.
///
/// Split out from [`resolve_nr_of_cores`] as a pure seam because
/// `available_parallelism()` cannot be made to fail from inside a test, leaving
/// the failure branch otherwise unverifiable. The result is always ≥ 1, and that
/// is what lets every downstream derivation drop its unknown-count special case.
fn core_count_or_default(detected: std::io::Result<NonZeroUsize>) -> u32 {
    detected.map_or(1, |cores| u32::try_from(cores.get()).unwrap_or(u32::MAX))
}
