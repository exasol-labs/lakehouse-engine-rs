//! Supported widening pairs; `arrow::compute::can_cast_types` is not used because it also
//! accepts narrowing casts no table format permits.
...
/// `None` when neither ordering is a supported widening.
...
/// `decimal(p, s)` → `decimal(p + k1, s + k2)` requires `k1 >= k2 >= 0`.
