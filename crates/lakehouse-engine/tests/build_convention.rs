...
#[test]
fn the_version_udf_suite_is_wired_into_make_test_e2e() {
    let workspace_root = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../..");

    let suite_file = workspace_root.join("crates/lakehouse-engine/tests/e2e_version_udf_test.rs");
    assert!(
        suite_file.exists(),
        "crates/lakehouse-engine/tests/e2e_version_udf_test.rs must exist on disk"
    );
    ...
    let recipe_line = lines
        ...
        .expect("Makefile must have a test-e2e target followed by a recipe line");
    assert!(
        recipe_line.contains("--test e2e_version_udf_test"),
        "test-e2e target must run --test e2e_version_udf_test"
    );
}
