//! E2E tests against a real ADLS Gen2 account, catalogued by a local Lakekeeper,
//! over the static (account key) and vended (SAS) credential arms. Storage is
//! real cloud because Azurite's `dfs` endpoint is incomplete and Lakekeeper's
//! `adls` profile can't address it. The container is created under an Entra ID
//! service principal because the Azure blob crate accepts no account key. Tests
//! share one Exasol provisioning and run with `--test-threads=1`.
...
fn setup() {
    SETUP_DONE.get_or_init(|| {
        ...
        // The warehouse is per-run and owned by `AzureFixture`, not created here.
...
struct AzureArm {
    /// Also the warehouse's `key-prefix`.
    warehouse: String,
    vs: &'static str,
...
/// Hold as a test-function local: `_container`'s `Drop` deletes the container on
/// return or panic, which a static would never run. That is why both arms share
/// one test rather than a `OnceLock`.
struct AzureFixture {
    account_name: String,
    ...
    /// Declared last so it drops after every field describing it.
    _container: AzureContainer,
}

impl AzureFixture {
    /// The container comes first because Lakekeeper probes it at warehouse
    /// creation; vended before static so a static failure can't hide vended
    /// evidence. Blocking calls stay outside `rt.block_on`, where they would panic.
    fn provision() -> Self {
        setup();

        // Read before creating anything, so a missing variable leaves no container.
        ...
        // Fetches a fresh short-lived Keycloak token immediately before each seed.
        ...
        // Empty S3 fields make the adapter read this as an Azure CONNECTION.
        let static_password =
...
/// The readback catches a create Lakekeeper silently rejected for overlapping a
/// sibling's storage profile, which would otherwise surface as an opaque seed error.
...
/// Seed shape: id 1..20, score = 5.0 * id.
...
fn azure_static_and_vended_creds_end_to_end() {
    ...
    // vended proof (its CONNECTION carries no account name/key).
...
/// Runs inside [`azure_static_and_vended_creds_end_to_end`] to share its container guard.
...
fn azure_container_guard_deletes_on_panic() {
    ...
    // `Handle::block_on` in a sync closure would itself panic.
    let container_name = azure::per_run_container_name();
