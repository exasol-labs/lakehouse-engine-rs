...
fn setup_e2e() {
    SETUP_DONE.get_or_init(|| {
        ...
        create_virtual_schema(&mut conn, SAMESHARD_VS_NAME, Some(1));
        create_virtual_schema(
            &mut conn,
            SPLITSHARD_VS_NAME,
            Some(SPLIT_PARALLELISM_FACTOR),
        );
    });
}
...
// e2e_scan_test.rs's local_stack_creds/local_stack_storage/local_stack_catalog.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Shared numeric parsers (small dup of the pattern in e2e_capability_test.rs)
// ---------------------------------------------------------------------------
...
fn fixture_spark_file_granularity_delete_table() {
    ...
    let files = resolve_fixture_files(FILE_GRANULARITY_TABLE);
    assert_eq!(
        files.len(),
        2,
...
fn fixture_spark_partition_granularity_delete_table() {
    ...
    let files = resolve_fixture_files(PARTITION_GRANULARITY_TABLE);
    assert_eq!(
        files.len(),
        4,
