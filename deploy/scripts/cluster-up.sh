...
# c4 runs from this machine, not a node: it SSHes to the nodes' external IPs, and a node cannot
# hairpin to its own public IP.
# KEY_FILE defaults to ~/.ssh/<key_pair_name>; if missing, it is fetched from SSM.
set -euo pipefail
...
# mktemp creates the file 0600, so the key is never group/world-readable, even transiently.
if [ ! -f "$KEY_FILE" ]; then
...
# c4 resolves CCC_HOST_KEY_PAIR_FILE by bare name in ~/.ssh.
mkdir -p "$HOME/.ssh"
...
# CCC_PLAY_DB_MEM_SIZE is the total over active nodes and must be set explicitly: left empty, this
# c4 build writes MemSize=0 and the DB never starts.
...
# auto_start can lag; an explicit start is idempotent.
