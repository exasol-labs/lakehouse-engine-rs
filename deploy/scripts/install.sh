...
print_next_step_template() {
  ...
  emit "  }';"
  emit ""
  emit "-- Grant the adapter and scan scripts access to the CONNECTION's credentials via a"
  emit "-- deployment-scoped ROLE, held by the principal that will OWN the virtual schema."
  emit "-- Exasol checks 'ACCESS ON CONNECTION ... FOR SCRIPT' against the VIRTUAL SCHEMA OWNER,"
  emit "-- not against each querying user (verified live) -- so this is ONE deployment-time"
  emit "-- grant, not one per reader. See docs/security.md."
  emit "--"
  emit "-- Run this BEFORE the CREATE VIRTUAL SCHEMA below: the adapter resolves the CONNECTION"
  emit "-- while creating the virtual schema, so a non-DBA owner without the LAKEHOUSE_ADAPTER"
  emit "-- grant cannot create it at all."
  emit ""
  if [[ "${installing_user,,}" == "sys" ]]; then
    emit "-- A SYS owner needs none of the statements above: it holds every CONNECTION"
    emit "-- implicitly, and Exasol refuses both GRANT forms to SYS outright anyway."
  else
  ...
  emit ""
  emit "-- Adding a READER thereafter is plain RBAC -- nothing connection-related, and NOT the"
  emit "-- role above (a reader holding it is neither necessary nor sufficient, verified live):"
  emit "--   GRANT SELECT ON SCHEMA <MY_LAKEHOUSE> TO <new-user>;"
  emit "--"
  emit "-- Grant $role only to a FURTHER VIRTUAL SCHEMA OWNER -- someone who will run"
  emit "-- CREATE VIRTUAL SCHEMA against this CONNECTION themselves:"
  emit "--   GRANT $role TO <other-vs-owner>;"
  emit ""
  emit "-- NOTE: a non-DBA owner also needs the CREATE VIRTUAL SCHEMA system privilege and EXECUTE"
  emit "-- on $schema.LAKEHOUSE_ADAPTER. A DBA owner (e.g. SYS) needs NEITHER the role nor the"
  emit "-- connection grant -- it holds every CONNECTION implicitly -- and Exasol REFUSES both"
  emit "-- GRANT forms to SYS outright (SQL state 42500), so skip this whole block as SYS:"
  emit "--   'cannot grant roles to SYS'"
  emit "--   'cannot grant connections to SYS'"
  emit ""
  emit "-- WARNING: CREATE ROLE has NO 'IF NOT EXISTS' form. Re-running this against an existing"
  emit "-- deployment fails if the role already exists -- check first:"
  emit "--   SELECT ROLE_NAME FROM EXA_ALL_ROLES WHERE ROLE_NAME = '$role';"
  emit "-- and skip the CREATE ROLE line if it returns a row."
  emit ""
  emit "-- WARNING: re-running 'CREATE OR REPLACE CONNECTION LAKEHOUSE_CATALOG_CREDS' -- or"
  emit "-- 'CREATE OR REPLACE SCRIPT $schema.LAKEHOUSE_SCAN' (e.g. on an engine upgrade/redeploy,"
  emit "-- which this installer performs on every re-run) -- DROPS the 'ACCESS ON CONNECTION ..."
  emit "-- FOR SCRIPT' grant, regardless of which grantee holds it. Re-issue BOTH GRANT ACCESS ON"
  emit "-- CONNECTION lines above (to the role) after either statement."
}
