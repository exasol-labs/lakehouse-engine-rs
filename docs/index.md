...
|-------|----------------|
| [Install](install.md) | One command installs the engine on any Exasol deployment: SaaS, AppDB, Docker, or on-premise. It uploads the `.so`, registers the Rust SLC, and creates the scripts. Build-from-source and fully manual paths are covered too, as appendices. |
