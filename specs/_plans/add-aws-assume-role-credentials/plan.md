...
6. A role changes only the reads of the key pair and leaves credential vending unchanged (decision-log [2]).

- **Goals**: role assumption for Iceberg REST (including Glue), Unity Catalog, and direct-storage CONNECTIONs. Session credentials for SigV4 signing and non-vended storage. One STS call per request. Sealed transport to the scan. Credential-safe errors. Both spike scenarios proven against the Docker Exasol container.
- **Non-Goals**: an ambient AWS credential chain (interview Q5). Session renewal for scans that outlive the 3600-second STS session. Configurable `DurationSeconds` or `RoleSessionName`. `AssumeRoleWithWebIdentity` or SAML. MinIO-native role semantics. Role assumption for Azure storage. Any change to credential vending.
...
ResolvedConnectionConfig { creds: effective, storage: storage_block(effective) }
  ...
  ├── format readers and access-delegation header: unchanged, branch on use_vended_credentials
  │      false ──▶ static storage (the session for a role)
  │      true  ──▶ resolve_vended_storage / Unity temporary credentials
  └── scan_storage_for: no vending and no role ──▶ ScanStorage::Connection (reference)
                        vending or role        ──▶ ScanStorage::Sealed ──▶ scan UDF unseals, never calls STS
```

`lakehouse-catalog` owns the AWS protocol: endpoint and region resolution, signing, and XML parsing. `lakehouse-engine` owns the Exasol delivery: the CONNECTION read, validation, the two entry points, and the scan wire variant. The dependency stays one-way, engine to catalog.
...
| One changed transport site | `scan_storage_for` | The wire variant already has one owner. A role adds one condition there, and no vending site changes. |
| Reuse the existing seams | `sign_request` (service `sts`), `seal_storage` | No second signer and no second credential transport. |

#### Quick Diagnostic (new module `sts.rs`)

| Question | Answer |
|----------|--------|
| Does a one-sentence summary capture each responsibility? | Yes. `sts.rs` turns a CONNECTION credential set into the AWS identity a request acts as. |
...
| Does exactly one module own each decision? | Yes. `sts.rs` owns the STS protocol, `scan_storage_for` owns the wire variant, and `sealed.rs` owns sealing. |
...
| The role replaces the key pair only where it is read, and vending is unchanged (decision-log [2]) | An assumed role wins over vending; reject role plus vending; a storage-source enum | Vending never reads the key pair for storage. `scan_storage_for` is the one changed site. |
...
| `aws_external_id` is optional and requires a role (decision-log [5]) | Require the role and the external id together | AWS lists `ExternalId` as optional, and a same-account role often requires none. |
...
- **New optional CONNECTION fields**: `aws_assume_role_arn`, `aws_external_id`, and `aws_sts_endpoint`. `aws_external_id` and `aws_sts_endpoint` each require a role. A role CONNECTION requires `access_key` and `secret_key`.
- **A role leaves credential vending unchanged.** With `use_vended_credentials`, the session signs only the SigV4 catalog requests, and storage resolves through vending as before. Without vending, the session is the S3 credential.
...
- **Local stack.** `docker-compose.yml` gains the `sts-stub` service and two MinIO users. `make test-e2e` gains one binary. `make test-e2e-unity` gains one test and starts `sts-stub`.
...
- [ ] 1.2 Add `quick-xml = { version = "0.39", features = ["serialize"] }` to `[workspace.dependencies]` and to `crates/lakehouse-catalog/Cargo.toml`. Confirm that `Cargo.lock` gains no `[[package]]` entry. Run `cargo deny check`.
- [ ] 1.3 Implement the request side of `crates/lakehouse-catalog/src/sts.rs`. [expert]
  ...
  - Build the percent-encoded `GET` query with the fixed `RoleSessionName`. Add `ExternalId` only when the set states `aws_external_id`. Sign it with `sign_request(…, "sts")`.
  ...
  - Test in `sts_tests.rs`: the request shape, a role without an external id, endpoint precedence, the plaintext gate, the timeout, and the no-role passthrough.
  - Recompute the signature from the received raw query for an external id holding `+=,.@:/-`, and assert that it matches.
- [ ] 1.4 Implement the response side of `sts.rs`.
  ...
  - Redact every error against the base `secret_key`, the base `session_token`, and the external id when stated.
...
- [ ] 1.5 Add a `session_tests.rs` integration test. Assert that the session from `resolve_aws_identity` signs the `loadTable` GET and the signed namespace enumeration:
  ...
  - for a role CONNECTION that sets `use_vended_credentials`, the `X-Iceberg-Access-Delegation` header on the session-signed `loadTable`
- [ ] 1.6 Re-export `resolve_aws_identity` from `lib.rs`. Edit `tests/catalog_public_surface.rs` to name it and the new fields. Keep its demotion assertions.

### B. Adapter identity wiring and sealed transport (`lakehouse-engine` adapter)
...
- [ ] 2.1 Parse the three fields in `parse_creds`. Add `validate_assume_role_creds` to `validate_creds`, and test it in `connection_tests.rs`:
  - an external id without a role is rejected, naming `aws_external_id`
  - a role alone and a role with an external id are accepted
  - the base key pair, with the no-ambient-credential message
  - `aws_sts_endpoint` requiring a role
  - the unchanged `path_style` guard skips a role CONNECTION that sets `use_vended_credentials`
- [ ] 2.2 Resolve the AWS identity once per request at both entry points. [expert]
...
- [ ] 2.3 Make `scan_storage_for` seal a role CONNECTION. [expert]
  - Return `ScanStorage::Connection` only when the set neither sets `use_vended_credentials` nor names `aws_assume_role_arn`.
  - Otherwise return the sealed variant, or the refusal without key material. The refusal stays vending-specific, because a role CONNECTION requires `secret_key`.
  - Test in `support_tests.rs`: a role without vending seals, the payload unseals field-for-field to the session backend, a role with vending seals the vended backend, and a join seals each side.
...
- [ ] 2.4 Pin that a role leaves the Delta reader's vended path unchanged. Extend `vending_without_a_vending_key_errors_and_never_falls_back_to_static` in `delta_format_reader_tests.rs` with a role case. No production code changes: the format readers keep branching on `use_vended_credentials`.
...
- [ ] 3.7 Add the Unity Catalog role scenario to `crates/lakehouse-engine/tests/e2e_unity_test.rs` under `unity-e2e`.
  - Add `sts-stub` to the `unity-up` target's `--wait` list, to the `e2e-unity` job's `--wait` set, and to the Exasol hosts loop that `docker-compose.unity.yml` re-declares.
  - Build a second CONNECTION from `local_stack_connection_password()` with the base user's key pair, the role, the external id, and the internal stub URL. Create a second virtual schema over it.
  - Query the delete-free Delta table. Assert the rows `unity_delta_delete_free_table_returns_its_rows` asserts, and a grown stub count: `unity_role_connection_reads_a_delta_table_through_the_session`.
  - The test panics when the stub is unreachable.
  - Add `build_convention.rs` test `unity_up_and_the_e2e_unity_job_start_the_sts_stub`.
...
- [ ] 5.1 Update `docs/catalogs.md`.
  - In § "Connection fields", document the three fields, the external-id rule, and the base-identity requirement.
  - Document the endpoint and region rule, `ALLOW_HTTP` for an `http` STS endpoint, and the China override.
  - Document that a role leaves credential vending unchanged, and the 3600-second session.
...
|-------|-------|------------|-----------|
| A: Catalog-crate AWS identity | 1.1-1.6 | — | spec deltas `vs-adapter/connection-credentials-assume-role` (request, endpoint, response, failure, identity, SigV4, vending scenarios) and `vs-adapter/catalog-crate-public-surface-extensions`; `crates/lakehouse-catalog/src/{creds,sts,lib}.rs`, `creds_tests.rs`, `sts_tests.rs`, `session_tests.rs`, `test_support_tests.rs`, `crates/lakehouse-catalog/tests/catalog_public_surface.rs`, `Cargo.toml`, `crates/lakehouse-catalog/Cargo.toml` |
| B: Adapter identity wiring and sealed transport | 2.1-2.4 | A (consumes `resolve_aws_identity`; shares the `connection-credentials-assume-role` delta, so it runs after A) | spec deltas `vs-adapter/connection-credentials-assume-role` (validation, once-per-request, storage, vending, sealing scenarios), `vs-adapter/connection-credentials`, `vs-adapter/connection-credentials-sigv4`, `vs-adapter/connection-credentials-direct-storage`, `vs-adapter/scan-spec-credential-reference`, `vs-adapter/rest-catalog-oauth-auth`; the recorded (unedited) `vs-adapter/pushdown-planning-cloud-credentials` and `vs-adapter/delta-table-planning` — see decision-log [8] and [2]'s Consequences for the known gap their recorded text leaves for a non-vended role CONNECTION; `crates/lakehouse-engine/src/adapter/{connection,mod}.rs`, `connection_tests.rs`, `adapter_tests.rs`, `crates/lakehouse-engine/src/adapter/pushdown/{support.rs,support_tests.rs,pushdown_tests.rs}`, `crates/lakehouse-engine/src/adapter/pushdown/format/delta_format_reader_tests.rs` |
| C: Local assume-role E2E | 3.1-3.7 | B (the suite passes only once the engine assumes roles) | spec delta `e2e-harness/assume-role-e2e`; `docker-compose.yml`, `docker-compose.unity.yml`, `scripts/sts-stub/`, `Makefile`, `.github/workflows/ci.yml`, `crates/lakehouse-engine/tests/{e2e_assume_role_test.rs,e2e_unity_test.rs,build_convention.rs,common/stack.rs,common/raw_parquet.rs}` |
...
|------|----------|--------|
| None | — | The plan adds `sts.rs` and adds one condition to `scan_storage_for`. No function, test, or module becomes unused. `storage_block` keeps both of its callers. |
...
|----------|-----------|---------------|-----------|
| An external id is accepted only beside a role | Unit | `crates/lakehouse-engine/src/adapter/connection_tests.rs`; `crates/lakehouse-catalog/src/creds_tests.rs` | `external_id_without_a_role_is_rejected`; `a_role_without_an_external_id_is_accepted`; `connection_creds_debug_redacts_the_external_id` |
...
| The AssumeRole request carries the role, the external id, and the base identity's signature | Integration, E2E | `crates/lakehouse-catalog/src/sts_tests.rs`; `crates/lakehouse-engine/tests/e2e_assume_role_test.rs` | `assume_role_request_is_a_signed_get_carrying_role_and_external_id`; `a_role_without_an_external_id_sends_no_external_id_parameter`; `external_id_characters_survive_signing`; `role_connection_reads_the_rows_the_base_identity_was_denied`; `a_wrong_base_secret_fails_create_with_signature_does_not_match` (the stub verifies the signature independently) |
...
| Session credentials are the storage credential under every catalog kind | E2E | `crates/lakehouse-engine/tests/e2e_assume_role_test.rs`; `crates/lakehouse-engine/tests/e2e_unity_test.rs` | `role_connection_reads_the_rows_the_base_identity_was_denied`; `direct_storage_role_connection_lists_and_reads_through_the_session`; `unity_role_connection_reads_a_delta_table_through_the_session` |
| A role leaves credential vending unchanged | Integration, Unit | `crates/lakehouse-catalog/src/session_tests.rs`; `crates/lakehouse-engine/src/adapter/pushdown/format/delta_format_reader_tests.rs`; `crates/lakehouse-engine/src/adapter/connection_tests.rs` | `assumed_session_signs_load_table_and_namespace_enumeration` (vending case: the access-delegation header); `vending_without_a_vending_key_errors_and_never_falls_back_to_static` (role case); `the_path_style_guard_skips_a_role_with_vending` |
...
| A non-vended CONNECTION configuring a store endpoint without stating path_style is rejected | Unit | `crates/lakehouse-engine/src/adapter/connection_tests.rs` | `endpoint_without_a_stated_path_style_is_rejected_naming_the_field`; `the_path_style_guard_does_not_fire_without_an_endpoint_or_under_vending`; `the_path_style_guard_skips_a_role_with_vending` |
...
| One pure function selects the wire variant and gates the sealed envelope | Unit | `crates/lakehouse-engine/src/adapter/pushdown/support_tests.rs` | `scan_storage_variant_follows_vending_and_the_role` |
...
| The AWS identity resolver extends the crate's public surface through an explicit reviewed edit | Unit | `crates/lakehouse-catalog/tests/catalog_public_surface.rs` | `aws_identity_resolver_is_public` |
| The assume-role binary and its STS stub are wired into the suite gate | Unit, E2E | `crates/lakehouse-engine/tests/build_convention.rs`; `crates/lakehouse-engine/tests/e2e_assume_role_test.rs`; `crates/lakehouse-engine/tests/e2e_unity_test.rs` | `make_test_e2e_runs_the_assume_role_binary`; `ci_e2e_job_waits_for_the_sts_stub`; `unity_up_and_the_e2e_unity_job_start_the_sts_stub`; `the_sts_stub_is_reachable_or_the_suite_fails`; `unity_role_connection_reads_a_delta_table_through_the_session` (panics when the stub is unreachable) |
...
| A Unity Catalog CONNECTION with static keys naming the role reads a Delta table through the session | E2E | `crates/lakehouse-engine/tests/e2e_unity_test.rs` | `unity_role_connection_reads_a_delta_table_through_the_session` |
...
| connection-credentials-assume-role (external id without a role) | `CREATE OR REPLACE CONNECTION` with `aws_external_id` and no `aws_assume_role_arn`, then `CREATE VIRTUAL SCHEMA` | Fails with an error naming `aws_external_id` and stating that it requires `aws_assume_role_arn` |
...
| assume-role-e2e (Unity Catalog) | `make test-e2e-unity` | `unity_role_connection_reads_a_delta_table_through_the_session` passes, and the suite reports 0 failures |
