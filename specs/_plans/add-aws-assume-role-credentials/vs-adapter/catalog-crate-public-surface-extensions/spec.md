...
<!-- DELTA:NEW -->
### Scenario: The AWS identity resolver and the storage credential source extend the crate's public surface through an explicit reviewed edit
...
* *THEN* the crate SHALL add to its public surface exactly ONE async function that resolves the AWS identity a request acts as, exactly ONE enum naming the three storage credential sources, and exactly ONE `ConnectionCreds` method returning that enum, re-exported at the crate root, and `ConnectionCreds` SHALL gain the `aws_assume_role_arn`, `aws_external_id`, and `aws_sts_endpoint` fields beside its existing `pub` fields
