...
- **Decision:** `build_join_sql` renders each post-join sort key over the key column's emitted expression, the same `render_join_select_item` output the select list uses. It wraps that expression in `nullif(<expr>, '')` when the emitted value is a string. Direction and NULL placement render through `SortKey::render_ordered`, the shared direction/NULL-placement seam.
- **Alternatives:**
...
- **Consequences:**
  ...
  - The emitted-empty-string-to-NULL step is inferred from the live `'' IS NULL` capture, not observed on a UDF emit. Task 2.8 verifies it live: it compares the broadcast result with the two-scan fallback, where Exasol itself joins and sorts. If the fallback ranks an emitted empty string as non-NULL, the `nullif` rule is wrong, and the implementer stops and escalates.
  - A `Float32` or `Float64` key ranks `CASE WHEN isnan(<expr>) THEN NULL ELSE <expr> END`. The recorded `sql-comprehension/vs-expression-translator-float-div` spec states that the `emit_batch` path emits a `NaN` as a silent NULL, tracked by the open issue `#246`. DataFusion ranks a native `NaN` as a number, so without this rule a `DESC NULLS LAST` shard keeps its `NaN` rows and cuts real top values. If a fix of `#246` changes the emitted value, this rule changes with it. The flat-scan top-N path shares this divergence (decision [6]).
  - The rule is architectural: a per-shard sort whose output a merge re-ranks must rank the value the merge sees, and the module that renders the emitted value owns that rule. Every future per-shard sort follows it, including a fix of the flat-scan path (decision [6]).
- **Promotes to ADR:** yes
