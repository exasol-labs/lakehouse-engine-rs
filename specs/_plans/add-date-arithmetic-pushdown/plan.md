...
Advertise and translate the nine Exasol date functions whose DataFusion 54 rendering matches Exasol
semantics, so date-heavy expressions push down instead of raw-scanning (Closes #107). Six functions
stay deferred with a named parity or session-state divergence; LAST_DAY is excluded because it is
not an Exasol function.
...
per-function semantic-parity research against the Exasol built-in function reference and the
DataFusion 54 (Arrow 58) function surface. Each supported function gets a translator arm emitting a
canonical DataFusion expression and an `FN_*` capability; each deferred function keeps falling
through with a documented reason. One listed name, `LAST_DAY`, is not an Exasol function at all
(issue #107 listed it in error) and is excluded from both subsets.
...
| Integer interval scaling | `ADD_*` arms | DataFusion 54 `Interval × integer` coercion is verified; float scaling is not |
| Documented fall-through | deferred arms | Session-state or unverified-parity functions stay unsupported, named in Background |

### Consequences

| Decision | Alternatives Considered | Rationale |
|----------|------------------------|-----------|
| Support 9, defer 6, exclude LAST_DAY | Advertise all as a block | The bar forbids advertising unverified parity; block advertising risks silently wrong results; LAST_DAY has no Exasol function or capability to advertise |
...
|----------|-------------|-------------------------|
| ADD_DAYS | Supported | `<x> + CAST(ROUND(<n>) AS BIGINT) * INTERVAL '1 day'` |
| ADD_WEEKS | Supported | `<x> + CAST(ROUND(<n>) AS BIGINT) * INTERVAL '7 day'` |
| ADD_HOURS | Supported | `<x> + CAST(ROUND(<n>) AS BIGINT) * INTERVAL '1 hour'` |
| ADD_MINUTES | Supported | `<x> + CAST(ROUND(<n>) AS BIGINT) * INTERVAL '1 minute'` |
| ADD_YEARS | Supported | `<x> + CAST(ROUND(<n>) AS BIGINT) * INTERVAL '1 year'` (leap clamp) |
| DAYS_BETWEEN | Supported | `CAST(<a> AS DATE) - CAST(<b> AS DATE)` |
...
| DAYOFWEEK | Deferred | Depends on `NLS_FIRST_DAY_OF_WEEK` session parameter; VS cannot observe it |
...
- DataFusion 54 (Arrow 58) — already the workspace version. Verified surface: `Interval × integer`
  coercion, `date_part('epoch', …) → Float64` seconds, `date_trunc` month/interval composition.
  Assumed surface, not yet source-confirmed: `DATE − DATE` yields an integer day count (not an
  interval type). The `DAYS_BETWEEN` E2E case (task 3.1) confirms or refutes this assumption; if
  the subtraction yields an interval, the `DAYS_BETWEEN` arm is withdrawn per the parity gate.

## Implementation Tasks

1. Translate the supported date functions and pin renderings
   - [ ] 1.1 Add translator arms to `crates/vs-expression/src/lib.rs` for `ADD_DAYS`, `ADD_WEEKS`, `ADD_HOURS`, `ADD_MINUTES`, `ADD_YEARS`, `DAYS_BETWEEN`, `HOURS_BETWEEN`, `MINUTES_BETWEEN`, and `SECONDS_BETWEEN` emitting the canonical renderings, with arity checks that fall through (raise / `None`) on wrong argument counts; add rendering unit tests pinning each emitted string [expert]
2. Align advertised capabilities with the translated set
   - [ ] 2.1 In `crates/lakehouse-engine/src/adapter/capabilities.rs`, advertise `FN_ADD_DAYS`, `FN_ADD_WEEKS`, `FN_ADD_HOURS`, `FN_ADD_MINUTES`, `FN_ADD_YEARS`, `FN_DAYS_BETWEEN`, `FN_HOURS_BETWEEN`, `FN_MINUTES_BETWEEN`, and `FN_SECONDS_BETWEEN`, moving these nine names in `reports_audited_capability_set` from the must-NOT list to a must-be-advertised assertion, and keep `FN_ADD_SECONDS`, `FN_ADD_MONTHS`, `FN_MONTHS_BETWEEN`, `FN_YEARS_BETWEEN`, `FN_DAYOFWEEK`, `FN_CONVERT_TZ` in the must-NOT-be-advertised assertion; in `crates/vs-expression/src/lib.rs` narrow the `unsupported_date_fn_falls_through` test to `ADD_SECONDS`, `ADD_MONTHS`, `MONTHS_BETWEEN`, `YEARS_BETWEEN`, `DAYOFWEEK`, `CONVERT_TZ`, `POSIX_TIME`, and `LAST_DAY`. `LAST_DAY` is a fall-through-only name (no translator arm, no `FN_LAST_DAY` — no such Exasol function or capability exists); it MUST NOT appear in any capabilities.rs must-be-advertised assertion. This single task owns all `reports_audited_capability_set` edits to avoid a concurrent-edit collision
3. Verify Exasol parity end to end
   - [ ] 3.1 Add end-to-end parity tests in `crates/lakehouse-engine/tests/e2e_capability_test.rs` executing each supported function through the VS against the seed table and asserting Exasol-matching values, with these cases pinned by literal arguments: (a) `ADD_YEARS` leap-year clamp `ADD_YEARS(DATE '2000-02-29', 1)` → `2001-02-28`; (b) round-half-away-from-zero `ADD_DAYS(d, 1.5)` → +2 days and `ADD_DAYS(d, 2.5)` → +3 days; (c) `ADD_HOURS` and `ADD_MINUTES` on a DATE-typed input (not only TIMESTAMP), confirming Date32→timestamp promotion in DataFusion; (d) one fractional `*_BETWEEN`, `HOURS_BETWEEN` between two timestamps 2.5 hours apart → `2.5`; (e) one negative-sign case, `DAYS_BETWEEN` with the first argument earlier than the second → a negative result. Withdraw the arm and its `FN_*` for any function whose result diverges from Exasol [expert]
...
|----------|-----------|---------------|-----------|
| ADD_DAYS, ADD_WEEKS, ADD_HOURS, and ADD_MINUTES translate to rounded integer interval addition | Unit | `crates/vs-expression/src/lib.rs` | `renders_add_fixed_length_interval_functions` |
| ADD_DAYS, ADD_WEEKS, ADD_HOURS, and ADD_MINUTES translate to rounded integer interval addition | Integration | `crates/lakehouse-engine/tests/e2e_capability_test.rs` | `e2e_add_fixed_length_functions_match_exasol` |
| ADD_YEARS translates to year-interval addition with leap-year clamping | Unit | `crates/vs-expression/src/lib.rs` | `renders_add_years_year_interval` |
| ADD_YEARS translates to year-interval addition with leap-year clamping | Integration | `crates/lakehouse-engine/tests/e2e_capability_test.rs` | `e2e_add_years_leap_clamp_matches_exasol` |
...
|---------|---------|-----------------|
| vs-expression-translator-date-fns | `cargo test -p vs-expression date` | Rendering unit tests for the ADD_* and *_BETWEEN arms pass; `unsupported_date_fn_falls_through` passes for the six deferred functions, POSIX_TIME, and LAST_DAY |
