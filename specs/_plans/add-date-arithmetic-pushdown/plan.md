...
| Defer `ADD_YEARS` | `arrow_cast(<months>, 'Interval(YearMonth)')` + year-interval add; epoch/`365.25`-day arithmetic | Exasol applies month-end stickiness the same as `ADD_MONTHS`: `ADD_YEARS(DATE '2001-02-28', 3)` → `2004-02-29` (live Exasol 2025.1.3), which the execution-safe, type-preserving `Interval(YearMonth)` add path does NOT reproduce (Arrow keeps the day → `2004-02-28`). The leap clamp alone (`2000-02-29`+1y → `2001-02-28`) IS reproducible; the stickiness is the blocker. Epoch-second arithmetic is not calendar-correct, and the return type is input-type-dependent |
...
| ADD_YEARS | Deferred | Month-end stickiness, same class as `ADD_MONTHS`: `ADD_YEARS(DATE '2001-02-28', 3)` → `2004-02-29` (live Exasol) diverges from the execution-safe `Interval(YearMonth)` add (Arrow → `2004-02-28`). The leap clamp alone is reproducible; stickiness is the blocker. Return type also input-type-dependent |
