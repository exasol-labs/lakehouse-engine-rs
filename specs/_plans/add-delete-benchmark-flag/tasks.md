...
## Group D — Validation
- [~] D.1 Docker: `BENCH_WITH_DELETES=1 make bench` authors `tpch_deletes`, runs full suite, delete-count sanity passes; confirm OFF-path output unchanged. ATTEMPTED, NOT COMPLETED: the live run stalled for 15+ min past the UDF `.so` build with zero further output (suspected `rtk` command-proxy hook buffering a long-running interactive script, compounded by a full cold host `cargo test` rebuild after an unrelated `cargo clean`); killed rather than left hanging. Not re-attempted in this pass — offline `selftest` (covers the namespace-switch/header/ratio-bounds logic this run would exercise live) and code review stand in as the automated evidence for this pass; a live run remains recommended before merge. See verification-report.md.
