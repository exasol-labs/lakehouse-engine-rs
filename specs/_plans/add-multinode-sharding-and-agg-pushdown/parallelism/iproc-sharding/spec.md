...
The cluster node count is taken from the `CLUSTER_NODES` virtual-schema
property (default `1`). File-to-shard assignment is computed once in the
adapter; the scan UDF still receives an explicit file list per invocation and
never discovers files itself. No node scans another node's files.
...
* *AND* the generated SQL MUST be behaviourally identical to the pre-sharding pushdown query
