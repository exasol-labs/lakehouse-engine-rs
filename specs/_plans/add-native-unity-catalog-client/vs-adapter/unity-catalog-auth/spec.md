...
* *GIVEN* resolved Unity Catalog CONNECTION credentials that supply a non-empty `token` and no OAuth client credentials
...
* *GIVEN* resolved Unity Catalog CONNECTION credentials that supply `client_id` and `client_secret` and no static `token`
