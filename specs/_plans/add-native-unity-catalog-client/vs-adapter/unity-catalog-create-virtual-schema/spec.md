...
* *THEN* the adapter SHALL list every table in that schema by calling the shared `CatalogClient` list operation on the constructed Unity Catalog client, and return one virtual table per listed table
* *AND* the adapter SHALL name each virtual table by flattening the segments below the configured namespace plus the table name with `__` and uppercasing the result through the shared case-fold site
* *AND* the Unity Catalog client SHALL source every listed table's columns from the single paginated `GET /tables` list sweep and MUST NOT issue a per-table `GET /tables/{full_name}` to obtain column metadata, because the list response already carries each table's columns inline
* *AND* the adapter MUST NOT read the Delta transaction log or resolve any snapshot, because this path stops at catalog metadata
...
* *GIVEN* a Unity Catalog table whose columns declare the Spark type names `BOOLEAN`, `INT`, `LONG`, `DOUBLE`, `STRING`, `DATE`, `TIMESTAMP`, and `DECIMAL(p,s)` with `p` at most 36 and `s` at most 36
...
* *AND* the source-tagged type descriptor the mapping reads SHALL carry the FULL parameterized Spark type — the type name plus precision and scale from the wire `type_precision`/`type_scale`, or `type_text` — so the `DECIMAL` case resolves to `DECIMAL(p,s)` from the descriptor's own `p` and `s` rather than from a bare `DECIMAL` type name that carries neither
...
* *GIVEN* a Unity Catalog table whose columns include an array, map, struct, binary, interval, or variant type, or a `DECIMAL` whose precision or scale exceeds 36
* *WHEN* the adapter builds the virtual table's column list
