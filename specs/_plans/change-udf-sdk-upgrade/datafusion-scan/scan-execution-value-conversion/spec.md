...
* *AND* the remaining variants SHALL map as `Double` to `Float64`, `Boolean` to `Boolean`, `Date` to `Date32`, `Timestamp { precision }` to `Timestamp(Microsecond, None)` at EVERY precision (`datafusion-scan/type-mapping-timestamp-precision`), and the three that remain — `String`, `Char` and `Unsupported` — to `Utf8`, which subsumes `Utf8View`/`BinaryView` normalization and preserves the behavior the removed type-string path gave an unrecognized declaration
