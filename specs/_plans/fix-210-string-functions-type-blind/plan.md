...
| … and its broadcast-join AND clause: the decline reached through `extract_join_projection` falls back over the union of both joined tables' columns | Unit | `crates/lakehouse-engine/src/adapter/pushdown/joins/rendering.rs` | `join_projection_string_fn_coerces_decimal_and_declines_double` |
