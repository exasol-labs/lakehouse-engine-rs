...
* **This delta corrects format-scoped naming and is issue #324. The short-circuit is unchanged.** The trigger was recorded as "Iceberg-level file pruning" and "the Iceberg data-file list". The check fires on ANY resolved-empty file list: `handle_pushdown` calls the shared `empty_result_sql` when the format reader returns zero files, whichever reader that was. Pruning an Iceberg table's manifests (`vs-adapter/pushdown-file-pruning`) and pruning a Delta table's `add` statistics (`vs-adapter/delta-file-pruning`) both reach it.
* **What prunes is format-specific; that everything was pruned is not.** This feature owns only the second fact — the shape of the response when the resolved list is empty — so naming a pruning mechanism in its trigger clause described a caller rather than the condition.
* **The concrete E2E scenarios keep naming their Iceberg fixture**, because the fixture really is an Iceberg table backed by MinIO. Only the pruning-level phrasing, which asserted where pruning happens for every request, is neutralized.
