...
- [ ] 1.2 Measure the GUARDED shape live against the `LHVS.GT_LINEITEM_SCAN` native oracle, in BOTH conjunct orders, and record whether native Exasol raises: `SELECT COUNT(*) FROM ... WHERE (L_LINENUMBER - 1) <> 0 AND 0 < L_ORDERKEY / (L_LINENUMBER - 1)` and the same conjuncts reversed. The divisor `(L_LINENUMBER - 1)` is zero on the ten `L_LINENUMBER = 1` fixture rows and one on the ten `L_LINENUMBER = 2` rows, so the guard `(L_LINENUMBER - 1) <> 0` excludes exactly the zero-divisor rows and the left conjunct's true ratio over the 20-row batch is 0.5, above `datafusion-physical-expr` 54.1's `PRE_SELECTION_THRESHOLD` of 0.2 — unlike the identically-zero `(L_LINENUMBER - L_LINENUMBER)` divisor, which the guard can never exclude anything from. Confirm from `EXPLAIN VIRTUAL` `PUSHDOWN_SQL` that both conjuncts reach the scan in one pushed filter, and record the pre-fix GUARDED row count and the pre-fix UNGUARDED row count (task 1.1's shape) separately for each conjunct order, so the guard is shown to actually remove rows. This is the shape a correct query today can start failing on after the fix, so the native answer decides whether the post-fix behaviour is a parity gain or a regression. Write the measured outcome into the `A division by zero inside a filter predicate fails the query rather than changing the row count` scenario of `specs/_plans/fix-float-div-predicate-divzero/sql-comprehension/vs-expression-translator-float-div/spec.md`, replacing the clause that says the outcome SHALL be measured.
...
- [ ] 4.5 Add an E2E test in `tests/e2e_scan_test.rs` for the GUARDED shape task 1.2 measured, `e2e_float_div_guarded_by_a_non_zero_conjunct_matches_the_measured_outcome`: use the same `(L_LINENUMBER - 1) <> 0 AND 0 < L_ORDERKEY / (L_LINENUMBER - 1)` shape (and its reversed conjunct order) task 1.2 measured, run both conjunct orders against the native oracle, assert the outcome task 1.2 recorded in the spec, and assert from `EXPLAIN VIRTUAL` that both conjuncts reach the scan in one pushed filter. If the outcome is a raise, the test pins the over-raise direction of tracked exception 1 rather than a parity claim.
...
- [ ] 4.10 File the three tracked-exception issues from decision-log entries [7], [8], and [12], then replace every `(#TODO-suppression)`, `(#TODO-stored-nan)`, and `(#TODO-scalar-fns)` token in `plan.md`, in `decision-log.md`, and in both spec deltas with the filed issue number in the `(#NNN)` form. `review/` files are historical records and are never edited by this task. The gate below covers only the two spec deltas — the artifacts the recorder actually merges — not this task's own prose or the Checklist row's command string. FAIL this task if `grep -rn 'TODO-suppression\|TODO-stored-nan\|TODO-scalar-fns' specs/_plans/fix-float-div-predicate-divzero/ --include='spec.md'` returns any line.
...
| Tracked-exception issues filed | `grep -rn 'TODO-suppression\|TODO-stored-nan\|TODO-scalar-fns' specs/_plans/fix-float-div-predicate-divzero/ --include='spec.md'` | No output. Every placeholder token in the two spec deltas has been replaced by task 4.10 with the filed issue number in the `(#NNN)` form |
</content>
</invoke>
