...
- **Non-Goals**
  ...
  - Restoring now-family pushdown with full time-zone fidelity is out of scope. That needs `SESSIONTIMEZONE`, `DBTIMEZONE`, and a statement-level timestamp anchor plumbed into `CommonScanSpec`, either over a new connect-back call from the adapter or as new scan-spec fields. Task 15 owns filing the GitHub issue that tracks it, and § Checklist gates on that issue existing before this plan is recorded (see decision-log [14]) (tracked in #263). Withdrawal leaves no correctness gap, so the issue tracks a lost optimization, not a deviation.
