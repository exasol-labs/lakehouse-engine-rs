...
3. **Move the catalog access code**
   - [ ] 3.0 Add `crates/lakehouse-catalog/src/test_support.rs` (`#[cfg(test)]`) declaring `base_creds() -> ConnectionCreds` and `static_storage() -> StorageProps`, copied verbatim from `crates/lakehouse-engine/src/adapter/pushdown/test_support.rs:23` and `:44`. The moved test modules open with `use super::super::test_support::*;` and call these fixtures 15 times (`credentials.rs`: 9 `base_creds`, 6 `static_storage`) and 2 times (`namespace.rs:375-376`); the engine-side `test_support.rs` CANNOT move, because it opens with `use super::*` and names `ProjectionItem`, `ScanSpec`, `CommonScanSpec`, `FileEntry`, `DeleteFileRef`, `build_scan_driving_sql`, `SCAN_UDF_NAME`, `relativize_shards_to_root`, and `crate::adapter::sharding::partition_files_by_bytes`. Leave every engine-side declaration in place for the engine tests that still use them; copy only the two fixtures the moved tests need. This task must land before 3.2, because the catalog crate does not compile until those 17 fixture calls resolve.
   ...
   - [ ] 3.3 Move `adapter/pushdown/namespace.rs` into `lakehouse-catalog::namespace` with its tests; `list_namespace_tables` and `parse_table_ident` become `pub`; repoint `adapter/mod.rs`, `pushdown/mod.rs`, `pushdown/joins/*`, and `file_resolution.rs`. Drop the `pub use namespace::list_namespace_tables;` re-export. [expert]
...
- 3.0 depends on 2.1 (it names `ConnectionCreds` and `StorageProps`) but not on 2.2, so it runs alongside 2.2 in Group C.
