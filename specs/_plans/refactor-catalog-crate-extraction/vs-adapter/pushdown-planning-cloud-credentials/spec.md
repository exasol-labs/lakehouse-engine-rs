...
* *AND* `resolve_vended_storage` SHALL be the ONLY vended entry point reachable from outside the `lakehouse-catalog` crate; all seven mechanism functions SHALL be crate-private, and `extract_vended_keys` and `merge_vended_into_storage` SHALL NO LONGER be reachable at `crate::adapter::pushdown::<name>` or from the `tests/` crate
