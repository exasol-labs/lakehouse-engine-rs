...
* The per-node core count `nr_of_cores` that both modes read comes from
  ...
  once per request and defaulting to `1` when the platform cannot report it. No VS
  property overrides it and it is not recorded in `adapterNotes`; see
...
* *WHEN* the adapter resolves the DataFusion threading configuration
* *THEN* the adapter SHALL set both `df_threads_per_udf` and `df_target_partitions` to `1`, through the ordinary `max(1, floor(nr_of_cores / udf_instances_per_node))` formula rather than a separate unknown-core branch
* *AND* the adapter SHALL still return a successful `createVirtualSchema` response
<!-- /DELTA:NEW -->
