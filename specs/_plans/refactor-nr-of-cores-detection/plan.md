...
- [ ] 2.5 Add `nr_of_cores_property_is_ignored`, building two property objects that differ only by a literal `"NR_OF_CORES": "999"` key and asserting every resolver returns identical values for both. This is the evidence for the `vs-adapter/create-virtual-schema-adapter-notes` clause that the adapter accepts an unrecognized property name and resolves every budget to the same value without it. It is also the only pin on backward compatibility for a `CREATE VIRTUAL SCHEMA` statement that still supplies the removed property, so it stands whether or not any scenario names that property.
