...
One finding is not cleanup. `STDDEV(<expression>)` produces an `AggregatePlan` with neither a `column` nor an `arg_expr`, passes `validate_agg_col_types` on a default type, gets three `EMITS` columns, and then renders `COUNT("")` in the scan, which DataFusion rejects. The whole query errors where a decline would have returned the correct answer natively. The issue flags this as "fold in if cheap"; this plan folds it in as an explicit decline rather than a better error message.
...
At most one user-visible change, and which of two branches it is depends on task 1.2's live capture, not on this paragraph. What code inspection establishes: a query selecting a statistical aggregate over an expression (`STDDEV(a + b)`, `VARIANCE(LENGTH(name))`) produces an `AggregatePlan` carrying neither a `column` nor an `arg_expr`, `validate_agg_col_types` admits it on the `DOUBLE PRECISION` default, three `EMITS` columns are sized from that default, and the scan renders `COUNT("")`, which DataFusion rejects. What code inspection does NOT establish is whether Exasol pushes that shape at all, and therefore whether any user ever reached the rejection.

Branch one, the capture shows an error: the change turns a failure into a result. The adapter declines the aggregate pushdown for that item, Exasol computes the statistic natively over the returned rows, and the query answers correctly and more slowly instead of erroring. Branch two, the capture shows the query already succeeds: Exasol never pushed the shape, NOTHING changes for a user, and the decline ships as a structural guard over a plan that was unreachable, so that an unenforced limit stays distinguishable from a regression. Task 1.2 rewrites this paragraph and the one above it into the single branch its capture establishes, before task 1.7 begins.
...
- [ ] 1.2 Reproduce the expression-argument statistical-aggregate failure against the local Docker Exasol container, per CLAUDE.md § Verification discipline: run `SELECT STDDEV(score + id) FROM <vs_table>` and `SELECT VARIANCE(score * 2) FROM <vs_table>` through the deployed virtual schema and record the verbatim error, plus the `EXPLAIN VIRTUAL` output showing whether Exasol pushes the shape at all. If the query SUCCEEDS, Exasol is not pushing the expression argument and task 1.7 becomes a structural guard rather than a bug fix — record that capture in the `aggregate-extensions` delta's Background and keep the decline, because an unenforced limit is indistinguishable from a regression. If it FAILS, record the error text. This task changes no code and gates task 1.7.

  Final step of this task, before task 1.7 begins: rewrite § Impact's first two paragraphs (the code-inspection statement and the two branches) and the `aggregate-extensions` delta's § Background bullets 2 and 3 to state the captured outcome, deleting the `TASK 1.2 CAPTURE PENDING` marker and the branch the capture ruled out. Both artifacts currently name BOTH branches on purpose, because neither outcome is established yet; leaving both in place would record a query failure that may never have occurred. The delta MUST NOT reach `/speq:record` while the pending marker or the losing branch is still in it.
...
- Group A → Group B (no production edit may land before the byte-identity baseline exists; 1.2 must capture the failure before 1.7 fixes it)
...
| vs-adapter/pushdown-planning-aggregate-extensions (GATE: task 1.2 capture) | `SELECT STDDEV(score + id) FROM LAKEHOUSE_VS.EVENTS;` before the change | Record the verbatim outcome. An error confirms the latent defect; a correct result means Exasol does not push the shape and task 1.7 is a structural guard — either way the capture goes into the delta's Background before the PR leaves draft |
| vs-adapter/pushdown-planning-aggregate-extensions | `SELECT STDDEV(score + id) FROM LAKEHOUSE_VS.EVENTS;` after the change | The correct standard deviation of `score + id`, computed by Exasol over returned rows. A `column "" not found` error means the decline did not take effect |

The two before-and-after diffs are the only checks that distinguish a byte-identical refactor from a merely equivalent one at the database boundary, because the golden fixtures pin the adapter's output while these pin what Exasol actually receives and returns. Run all six before the PR leaves draft.
