...
|----------|-----------|---------------|-----------|
| Pushdown reads the cluster node count from the UDF handshake | Unit | `crates/lakehouse-engine/src/adapter/mod.rs` | `cluster_nodes_from_context_passes_through_reported_node_count` |
...
One half of the handshake-source scenario carries no automated test, deliberately. The positive half (`node_count()` is read, and `0` floors to `1`) is unit-tested above. The negative half — a persisted `CLUSTER_NODES` note can never become the shard-count source — is unobservable below the database: the derived count is visible only in the generated SQL, which `handle_pushdown` reaches after `CatalogSession::resolve` and `resolve_file_list`, so any test asserting it needs a live catalog and object store. Two non-test guards carry it instead. `cluster_nodes: usize` is a required parameter of `handle_pushdown_request` whose single caller sources it from `ctx.node_count()`, so a note read shadowing the parameter leaves it unused and fails CI's `cargo clippy --workspace --all-targets -- -D warnings`; dropping the `dispatch` capture fails the same gate on an unused `cluster_nodes_from_context`. Both were verified by injecting the regression. The one shape neither the lint nor any unit test detects is a note read that falls back to the parameter (`unwrap_or(cluster_nodes)`), which the § Manual Testing legacy-key and four-node staging rows exist to catch. A unit test that computes the shard count in its own body instead of calling production would restate these literals, not guard them.
