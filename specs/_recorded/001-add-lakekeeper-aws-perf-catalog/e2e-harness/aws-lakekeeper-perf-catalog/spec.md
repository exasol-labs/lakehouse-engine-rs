...
* The engine already supports Lakekeeper: Iceberg REST plus OAuth2 client-credentials, documented in
  ...
  `e2e-harness/lakekeeper-e2e-harness` against a local Docker stack. This feature adds **no** engine,
...
* *AND* the realm name, OAuth2 client id, client secret, and audience SHALL be the values
  ...
  CONNECTION carries the same field set `e2e-harness/lakekeeper-e2e-harness` already proves and
...
* *AND* the OAuth2 client secret SHALL remain the repo-committed value in
  ...
  contract `e2e-harness/lakekeeper-e2e-harness` proves is defined by that file and a second generated
