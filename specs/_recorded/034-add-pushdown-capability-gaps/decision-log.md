...
- **Evidence:** Confirmed against the Exasol engine source (not memory): `[redacted]/[redacted]` (`[redacted]`) sets `root["type"] = "function_scalar_cast"`, `root["name"] = "CAST"`, `root["dataType"] = …`, `root["arguments"] = [<src>]`, and is the **sole** CAST emitter. Sibling nodes emit `"function_scalar_case"` ([redacted]) and `"function_scalar_extract"` ([redacted]) the same way. REGEXP_LIKE is emitted only as `predicate_like_regexp` ([redacted]), confirming the `function_scalar`+REGEXP_LIKE arm is likewise a defensive-only alternate encoding — the precedent followed here for keeping the nested CAST arm.
