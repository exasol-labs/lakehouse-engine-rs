...
- **Why superseded:** Verified in exasol-db/script-languages: groups drive UDF *invocations*, not OS processes; parallel instances on a node are a VM pool sized to `NR_OF_CORES` (`primitives.cpp:267`, `swigengine.cc:1147-1184`); `GROUP BY IPROC()` (`misc_primitives.cpp:98-132`) yields exactly one group per node → caps parallelism at the node count and leaves a node's other cores idle.
- **Promotes to ADR:** no
