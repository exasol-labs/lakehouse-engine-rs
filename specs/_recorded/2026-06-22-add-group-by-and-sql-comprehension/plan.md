...
v0.3.0 ships single-group aggregate pushdown (COUNT/SUM/MIN/MAX/AVG with partial/merge decomposition per ADR-008) and IPROC fan-out (ADR-007). A corrected reading of the Exasol engine internals (verified in `exasol-db`/`script-languages`) invalidates two pillars of the prior plan:

- **Parallelization (was ADR-007).** Groups drive UDF *invocations*, not OS processes; actual parallel instances on a node are a fixed VM pool sized to `NR_OF_CORES` (`primitives.cpp:267`, `swigengine.cc:1147-1184`), and groups are multiplexed onto it (`set_function.cpp:240-260`). `GROUP BY IPROC()` (`misc_primitives.cpp:98-132`) yields one group per node → caps parallelism at node count and leaves a node's other cores idle. Group→node distribution is round-robin when group count ≤ `max_dynamic_group_count` (default 300), hash-partitioned above (`globalgroupbyset5.cpp:295-341`).
- **Memory safety (was the file-count cardinality guard).** Per-instance memory is the per-process heap (default 4096 MB) enforced via `setrlimit(RLIMIT_RSS)`; the dispatcher stalls additional concurrent VMs once usage hits 80% of the limit (`swigengine.cc:1574-1595`). The limit is available in UDF metadata (bytes) via the sibling-repo accessor `language-container-rs:add-memory-limit-metadata` (`UdfContext::memory_limit() -> u64`; `0` = unbounded/unknown).
...
- [ ] 4.2 Add the UDF parallelization & memory model section to `CLAUDE.md` with the exasol-db citations (done in this revision — verify wording)
