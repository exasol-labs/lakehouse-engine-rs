...
Fulfils the post-merge re-bench below. Cluster `203.0.113.10` (3 nodes n11/n12/n13, DB 2025.1.11); AWS Glue catalog, namespace `tpch` ≈ SF10 (`lineitem` ≈ 60M rows / 10 data files / ~1.7 GB; `orders` ~861 MB). SLC **lc-rs 0.19.1** (released), `.so` built against `exasol-udf-sdk` 0.19.1 (matching fingerprint), `NR_OF_CORES=4`, `PARALLELISM_FACTOR=8`.
