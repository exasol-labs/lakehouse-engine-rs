...
- **Live re-bench** on the AWS Glue cluster (`203.0.113.10`) of the two previously-crashing queries (grouped aggregate over full `lineitem`; ~60M-row join) to confirm they now complete or return a clean `ResourcesExhausted` rather than `VM crashed`, and to capture per-query wall-clock + `PROFILE`. This is the deliverable the bounding work unblocks; it runs against the live cluster after merge.
