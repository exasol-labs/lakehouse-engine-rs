...
| IPROC / NPROC | `IPROC()` = node number, `NPROC()` = active node count. The shard-count node number is read from `UdfContext::node_count()` per pushdown request, not from `NPROC()` over connect-back; sharding does NOT group on `IPROC()` (that would cap parallelism at the node count) |
