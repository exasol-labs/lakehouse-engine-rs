...
an aggregate before, between, or after the group keys in the `selectList` — the
arrangement every pre-existing GROUP BY E2E case avoided, which is how the outer-SELECT
column transposition bug (#33) shipped undetected.
