# Feature: VS Expression Translator — Date-Difference Functions

Extends the VS expression translator (`sql-comprehension/vs-expression-translator`) with the
Exasol date-difference (`*_BETWEEN`) function family, plus the divergent date-arithmetic
functions issue #107 examined but found no faithful DataFusion 54 rendering for. Split out of
`sql-comprehension/vs-expression-translator-date-fns` to keep that feature's scenario count
within the domain's convention (mirroring the existing `-scalar-fns`/`-scalar-ops`/`-literals`
splits).
...
* This feature shares the six public entry points of `crates/vs-expression` — the DataFusion trio
  ...
  the date-difference functions are additional arms inside the same recursive walker.
* Field-shortcut date functions, `EXTRACT`, `DATE_TRUNC`, `TO_DATE`/`TO_TIMESTAMP`, `WEEK`, and the
  now-family withdrawal are covered by the sibling feature `sql-comprehension/vs-expression-translator-date-fns`.
  This feature covers only the date-difference family and the functions issue #107 examined but
  declined to translate. Both features share the same Exasol-dialect verbatim rule and the same
  `TRANSLATED_SCALAR_FNS` declaration mechanism, described once in
