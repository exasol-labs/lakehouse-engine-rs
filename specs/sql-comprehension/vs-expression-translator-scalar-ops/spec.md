...
* *GIVEN* a VS expression node of type `function_scalar` with `name` equal to `ADD`, `SUB`, `MUL`, `FLOAT_DIV`, or `NEG`
* *WHEN* `render_expression` processes the node
* *THEN* binary arithmetic nodes (`ADD`, `SUB`, `MUL`, `FLOAT_DIV`) SHALL return `(<left> <op> <right>)` where the operators are `+`, `-`, `*`, `/` respectively
* *AND* unary negation (`NEG`) SHALL return `(-<operand>)`
