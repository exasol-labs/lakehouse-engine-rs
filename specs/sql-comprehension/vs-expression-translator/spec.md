...
| `render_df_filter_qualified` (`adapter/pushdown/joins/rendering.rs`) | the outer WHERE residual of the N-scan join wrapper, and the outer WHERE of the qualified single-table wrapper when the single-table DataFusion filter render declined |
