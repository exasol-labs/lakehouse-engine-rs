...
* There is no snapshot, no transaction log, and no manifest. The reader's whole job is to turn a
  table root plus a merge mode into the resolved scan every other reader returns.
...
* The reader owns NO listing and NO footer parsing of its own: both come from
  `vs-adapter/parquet-directory-seam`, the same seam table enumeration reads.
...
* This reader has no catalog statistics to prune from, so its plan-time file list is every file
  under the table root. Path-based partition pruning is issue
  [#408](https://github.com/exasol-labs/lakehouse-engine-rs/issues/408). Footer-statistics
  pruning is issue [#412](https://github.com/exasol-labs/lakehouse-engine-rs/issues/412). Both are
  recorded here as explicit tracked exceptions rather than left as unstated gaps.
...
* *THEN* the scan source SHALL gain a THIRD variant carrying the request's object store, the table root, and the resolved merge mode, and the selection site SHALL match all three variants EXHAUSTIVELY, so a fourth table format is a compile error there rather than a silent fall-through
...
* *GIVEN* a direct-storage table root holding Parquet data files at more than one depth, and the merge mode the virtual schema resolved
* *WHEN* the direct-storage format reader resolves that table's scan
* *THEN* the reader SHALL obtain the file list, each file's byte size, and the folded schema from the ONE shared directory seam, and MUST NOT list objects or parse a footer itself
* *AND* each returned file entry SHALL carry its path and its byte size, an EMPTY delete-mechanism list, and an EMPTY per-file partition-value map, so the serialized per-shard file list takes the same compact form an unpartitioned delete-free Iceberg scan already produces
...
* *AND* the returned scan SHALL carry EMPTY partition columns and an EMPTY name-mapping list, because a raw directory declares neither
