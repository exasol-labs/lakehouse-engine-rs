...
* *AND* the scan-driving SQL generated for a given pushdown request MUST be byte-identical to the pre-refactor output EXCEPT for that `storage` VALUE, which `vs-adapter/scan-spec-credential-reference` re-encodes as a connection reference carrying no credential or as a sealed envelope over the backend's own encoding — a WIDENING of the recorded variant-tag exception to the granularity the clause above already uses, because the value itself now changes rather than only what encloses it; the backend's own encoding stays byte-identical either way, per `vs-adapter/storage-backend-enum`
