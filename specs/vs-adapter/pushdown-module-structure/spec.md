...
### Scenario: Two ordered pipeline functions own the type-rewrite pass order
...
* *THEN* exactly one function in the shared `support` submodule SHALL own each pass sequence — one for the filter tree and one for the select-list item — and every caller SHALL invoke that function instead of sequencing the passes itself, so adding or reordering a pass is an edit inside one function body rather than an edit at every caller
* *AND* each pipeline function's doc comment SHALL state its pass order AND why that order is load-bearing, so the rationale lives with the code that enforces it rather than beside one caller
* *AND* the select-list pipeline SHALL omit the LIKE-subject pass, and its doc comment SHALL record that omission as a gap tracked by issue #219 — a select-list item CAN carry a non-string-subject `predicate_like` and still hit the DataFusion coercion failure — citing `(#219)` inline rather than stating the omission is correct by construction, and the two pass lists SHALL remain two distinct functions rather than one function taking a pass-selection parameter
* *AND* each pipeline function SHALL take the expression tree and the column-type list and SHALL absorb the passes' fallibility disagreement behind one `Option`-returning signature — `None` meaning the tree declined, propagated unchanged from whichever pass declined, preserving the filter path's whole-filter decline and the select-list path's full-row fallback — so no caller composes a fallible pass against the infallible rewriter, and SHALL NOT absorb its caller's "is there a tree at all" question, keeping an absent filter distinguishable from a declined one
* *AND* all three passes SHALL narrow to private once the relocation removes their callers outside `support`, so no module outside `support` can sequence them by hand and the pipeline functions become the only reachable entry points while each pass stays directly callable from `support`'s own tests, and each pipeline function SHALL itself be declared at the narrowest visibility that compiles — `pub(super)` for the filter pipeline, which `pushdown/mod.rs` calls, and private for the select-list pipeline, whose only caller lives in `support`
* *AND* neither pipeline function SHALL name what its caller does with the result — not the Iceberg file-pruning call, not the full-row fallback flag, and not the SQL renderer — so the pipeline owns the pass order and nothing else, and the raw pushdown filter tree forwarded to Iceberg-level file pruning MUST remain unmodified because the pipeline feeds only the DataFusion-bound scan filter
* *AND* the scan-driving SQL generated for every pushdown request MUST be byte-identical to its pre-extraction output, and every existing pushdown unit test, join golden-SQL assertion, and `dispatch_golden` fixture MUST pass with no change to any test assertion or expected value — the filter-chain tests in `pushdown/mod.rs` proving this by calling the extracted pipeline while keeping their rendered-SQL assertions unedited
