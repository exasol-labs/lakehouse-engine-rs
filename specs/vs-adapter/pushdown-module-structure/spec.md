...
* Ordering the three guards into a pass pipeline is NOT part of this scenario. The one
  production chain site that sequences them (`like` → `string` → `decimal`) keeps its explicit
  composition and its load-bearing order comment.
