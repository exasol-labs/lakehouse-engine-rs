...
* `string_function_arg_type_guard` is a post-order recursive walk returning `Option<Json>`: `Some(tree)` means render this (possibly rewritten) tree, `None` means decline. It recurses through every child-bearing field of this codebase's expression grammar — the array fields `expressions` / `arguments` / `results` and the single-child fields `expression` / `pattern` / `left` / `right` / `basis` — because a filter-side string function sits under a comparison predicate (`UPPER(c) = 'X'` is `predicate_equal` with the function under `left`), which `like_subject_type_guard`'s junction-only recursion never reaches.
...
* *AND* a governed string function used AS a LIKE subject — for example `UPPER(c_decimal_a) LIKE '1%'`, which `like_subject_type_guard` leaves alone as a non-bare-column subject — SHALL be coerced by `string_function_arg_type_guard`'s deeper recursion
