...
Pushes an `ORDER BY <cols> LIMIT n` query (single table, no GROUP BY, no aggregate,
no OFFSET) down as a decomposed partial/merge top-N: each shard computes its own
bounded local top-N over only its assigned files and emits at most `n` rows, and
Exasol merges the per-shard rows with a final `ORDER BY <cols> LIMIT n`. This
replaces the raw-emit-the-whole-table-then-sort-in-Exasol path for the common
"top-N by a sort key" query, so only `≤ (shard_count × n)` rows cross the UDF
boundary instead of every matching row.
...
* *GIVEN* a single-table `pushdown` request with no group keys, no aggregate select items, and no `having`, whose `order_by` entries are all bare column references present in the projection and whose `limit` carries `numElements = n` with no offset
* *WHEN* Exasol sends the `pushdown` request
* *THEN* the adapter SHALL carry the sort key list (each with its column, direction, and NULL placement) and the row limit `n` into the shard-invariant common scan spec so every row-scan shard invocation computes the SAME bounded local `ORDER BY <keys> LIMIT n`
* *AND* the generated scan-driving SQL SHALL wrap the shard fan-out in an outer `SELECT <projection> FROM (<fan-out>) ORDER BY <keys> LIMIT n` — using the identical key order, direction, and NULL placement as the per-shard sort — so the merged result equals the same `ORDER BY <keys> LIMIT n` evaluated over all matching rows on a single node
